    ...
    div.innerHTML = `
      <div>
        ...
        <span class="item-price">$${(
          item.dessert.price * item.quantity
        ).toFixed(2)}</span>
      </div>
      <div class="qty-controls">
        <button class="qty-decr" data-id="${
          item.dessert.id
        }" aria-label="Decrease">−</button>
        <span class="qty-value">${item.quantity}</span>
        <button class="qty-incr" data-id="${
          item.dessert.id
...
function showOrderModal() {
    ...
    row.innerHTML = `
      <div class="left">
        ...
        <div class="meta">
          ...
          <div class="qty">${it.quantity}x  @ $${it.dessert.price.toFixed(
      2
    )}</div>
        </div>
      </div>
  ...
  footer.innerHTML = `
    <div class="modal-total"><span>Order Total</span><span>$${cart
      .getTotal()
      .toFixed(2)}</span></div>
